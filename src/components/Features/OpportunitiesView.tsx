            ...
            <div className="flex flex-col md:flex-row justify-between items-end gap-4">
                ...
                <div className="bg-white/5 p-1 rounded-xl flex gap-1">
                    <button
                        ...
                        className={`px-4 py-2 rounded-lg text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'COMPETITION'
                    ...
                    <button
                        ...
                        className={`px-4 py-2 rounded-lg text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'SCHOLARSHIP'
                        ...
                        opportunities.map((op, idx) => (
                            ...
                            >
                                <div className="p-6 flex flex-col md:flex-row gap-6">
                                    {/* Icon Box */}
                                    <div className={`w-16 h-16 rounded-2xl flex items-center justify-center shrink-0 shadow-lg ${op.type === 'SCHOLARSHIP' ? 'bg-gradient-to-br from-green-500/20 to-emerald-500/20 text-green-400 border border-green-500/30' :
                                            'bg-gradient-to-br from-purple-500/20 to-indigo-500/20 text-purple-400 border border-purple-500/30'
                                        }`}>
                                        {op.type === 'SCHOLARSHIP' ? <School className="w-8 h-8" /> : <Trophy className="w-8 h-8" />}
                                    </div>
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-gray-400 text-sm mb-4 line-clamp-2">{op.description}</p>

                                        <div className="flex items-center justify-between mt-auto">
                                            <div className="flex gap-2">
                                                {op.tags?.map(tag => (
                                                    <span key={tag} className="text-xs text-gray-500 bg-white/5 px-2 py-1 rounded border border-white/5">#{tag}</span>
                                                ))}
                                            </div>
                                            ...
                                            {op.link && (
                                                <a
                                                    ...
                                                    className={`
                                                        flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all text-black
